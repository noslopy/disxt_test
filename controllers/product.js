...
const jwt = require('jsonwebtoken');
 ...
 exports.createProduct = async (req, res, next) => {
  try {
    ...
    const newProduct = new Product({ name, price, description, created_by: res.locals.loggedInUser._id });
    await newProduct.save();
    const accessToken = jwt.sign({ userId: res.locals.loggedInUser._id }, process.env.JWT_SECRET, {
      expiresIn: "1d"
    });
    res.status(200).json({
