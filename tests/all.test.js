...
describe("Endpoints", () => {
  ...
  it("Login admin user", async (done) => {
    const res = await request
      .post("/login")
      .query({ username: "asd@asd.com", password: "password1" });
  ...
  it("Update product with admin user", async (done) => {
    const res = await request
      ...
      .set("x-access-token", admin_token)
      .query({ description: "the item is awesome" });
    expect(res.status).toBe(200);
  ...
  it("Login client user", async (done) => {
    const res = await request
      .post("/login")
      .query({ username: "asd1@asd1.com", password: "password2" });
    expect(res.status).toBe(200);
